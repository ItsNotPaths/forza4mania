"""Download NadeoImporter and Blendermania_Dotnet from blendermania-assets.

The blendermania-addon team mirrors both binaries on a public GitHub
release page (https://github.com/skyslide22/blendermania-assets/releases).
We piggyback on that mirror — they have license to distribute, we don't.

If the mirror disappears, fall back instructions point users to nadeo.com
(for NadeoImporter) and the blendermania-dotnet repo's own releases (for
the dotnet helper).
"""
...
NADEO_IMPORTER_URL = (
    f"{_GITHUB_ASSETS}/NadeoImporter_{NADEO_IMPORTER_VERSION}/{NADEO_IMPORTER_VERSION}.zip"
)
